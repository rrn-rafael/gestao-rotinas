...
- All implementation work must happen on `codex/teste`.
- If work is found on any other branch, move it to `codex/teste` before continuing.
...
- Before saying a change is ready for testing, stage, commit, and push the validated candidate to `origin/codex/teste`.
- Only treat `codex/teste` as test-ready when the latest intended changes are already published on the remote branch.
- If commit or push did not happen, explicitly say that `codex/teste` is not ready for testing yet.
- After local validation and publication to `origin/codex/teste`, stop and wait for the user's approval before any promotion step.

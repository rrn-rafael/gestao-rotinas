import type { ViewState } from "./types";

export const CARD_WIDTH = 208;
export const CARD_HEIGHT = 104;
export const CARD_GAP = 16;
export const MAP_HORIZONTAL_PADDING = 32;
export const MAP_VERTICAL_PADDING = 32;
export const TIMELINE_HEADER_DIVIDER_HEIGHT = 2;
export const TIMELINE_TOP_BAR_HEIGHT = 60;
export const TIMELINE_LABEL_BAND_HEIGHT = 38;
export const TIMELINE_HEADER_HEIGHT =
  TIMELINE_TOP_BAR_HEIGHT +
  TIMELINE_LABEL_BAND_HEIGHT +
  TIMELINE_HEADER_DIVIDER_HEIGHT;
export const TIMELINE_START_HOUR = 8;
export const TIMELINE_END_HOUR = 16;
export const TIMELINE_BUCKET_SAFE_PADDING = 18;
export const TIMELINE_BUCKET_LANE_GAP = 24;
export const TIMELINE_COLUMN_MIN_WIDTH =
  CARD_WIDTH + TIMELINE_BUCKET_SAFE_PADDING * 2;
export const TIMELINE_COLUMN_EMPTY_WIDTH = 132;
export const TIMELINE_COLUMN_SINGLE_WIDTH = 184;
export const TIMELINE_COLUMN_DENSITY_STEP = 44;
export const TIMELINE_COLUMN_MAX_WIDTH = 560;
export const TIMELINE_HOME_VIEW_PADDING = 24;
export const MIN_SCALE = 0.15;
export const MAX_SCALE = 1.8;

export const INITIAL_VIEW: ViewState = {
  x: 0,
  y: 0,
  scale: 1,
};

export const DEFAULT_SELECTED_CARD_ID: string | null = null;

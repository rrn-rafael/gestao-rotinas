...
- all in-progress implementation must live on `codex/teste`
- if work is discovered on another branch, move it to `codex/teste` before continuing
- before handing off for testing, stage, commit, and push the validated candidate to `origin/codex/teste`
...
- remain on `codex/teste` after validation and wait for approval before any promotion step
...
- if a candidate is rejected, delete or recreate `codex/teste` from the last approved baseline before starting new work
- if a candidate is approved, promote `codex/teste` to `main` only after explicit user approval
